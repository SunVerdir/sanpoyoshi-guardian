...
**Agentic AIによる規格外食材マッチング × 子ども食堂向け自律型監査エージェント** （第5回 Agentic AI Hackathon with Google Cloud 応募プロジェクト）

> **Elevator Pitch** Metaマルシェに出品される規格外食材を、行政が公費で買い取り子ども食堂を支援する——そんな公的な仕組みを想定した政策提言PoCです。「どの食堂に、どれだけ分配するか」という意思決定をエージェントが支援し、最終判断は必ず行政職員が下します（Human-in-the-loop）。分配の履歴は改ざん検知可能な台帳に記帳され、透明性を担保します。
...
🚀 **[稼働中のデモ（職員向け承認ダッシュボード）](https://sanpoyoshi-guardian-dashboard-e3sjywoowa-an.a.run.app)**（承認待ちのサンプル分配案を2件、事前に登録しています）
🎥 **[デモ動画（YouTube）](https://youtu.be/UsXuCaalj-8)**
...
[![システムアーキテクチャ図](https://github.com/SunVerdir/sanpoyoshi-guardian/raw/main/docs/architecture.png)](/SunVerdir/sanpoyoshi-guardian/blob/main/docs/architecture.png)
